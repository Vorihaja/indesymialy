  ...
  return (
    <ErpAuthGuard>
      <ErpShell
        ...
        stats={stats}
        actions={<button className="rounded-2xl bg-amber-500 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-amber-400 transition">Ajouter une offre</button>}
      ...
      <section id="offres" className="rounded-3xl border border-white/10 bg-slate-950/80 p-6">
        <div className="mb-6">
          <h2 className="text-xl font-semibold text-white">Offres actives</h2>
          <p className="text-sm text-slate-400">Suivez vos produits et promos dans la boutique.</p>
        </div>
        <div className="space-y-4">
          {offers.map((offer) => (
            <div key={offer.title} className="rounded-3xl border border-white/5 bg-slate-900/80 p-4 flex items-center justify-between gap-4">
              <div>
