        ...
        <aside className="fixed bottom-0 left-0 top-[var(--platform-header-height)] z-20 w-64 overflow-hidden border-r border-zinc-900 bg-zinc-950 flex flex-col justify-between p-4">
          <div className="flex min-h-0 flex-1 flex-col">
            ...
            <nav className="mt-6 min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
              {sidebarItems.map((it) => (
                <button key={it.id} onClick={() => { setCurrentModule(it.id); if (it.id === "events") setSelectedEvent(null) }} className={`flex w-full items-center gap-3 whitespace-nowrap px-4 py-3 text-xs font-semibold tracking-wide border-l-2 transition-all ${currentModule === it.id ? "bg-amber-500/10 text-amber-400 border-amber-500" : "text-zinc-400 border-transparent hover:bg-zinc-900"}`}>
                  {it.icon}{it.label}
                </button>
              ))}
            </nav>
